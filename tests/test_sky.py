...
def test_suns(tmpdir):
    sm = SkyMapper((46.25, -6.13, -15), sunres=8)
    ...
    sm = SkyMapper('test_suns.txt', sunres=8)
    suns2 = sm.solar_grid(False, 1)
    assert np.allclose(suns, suns2, atol=.0001)
    sm = SkyMapper(suns, sunres=8)
    suns2 = sm.solar_grid(False, 1)
    assert np.allclose(suns, suns2, atol=.0001)
    sm = SkyMapper("geneva.wea", sunres=8)
...
def test_sunrotation(tmpdir):
    sm = SkyMapper((46.25, -6.13, -15), skyro=127, sunres=8)
    suns = sm.solar_grid(False, 1)
    ...
    sm = SkyMapper('test2_suns.txt', sunres=8)
    suns2 = sm.solar_grid(False, 1)
    assert np.allclose(suns, suns2, atol=.0001)
    sm = SkyMapper(translate.rotate_elem(suns, -127), skyro=127, sunres=8)
    suns2 = sm.solar_grid(False, 1)
    assert np.allclose(suns, suns2, atol=.0001)
    sm = SkyMapper("geneva.wea", skyro=127, sunres=8)
    suns2 = sm.solar_grid(False, 3)
