...
def tmpdir(tmp_path_factory):
    ...
    shutil.copytree('tests/skysampler/', data + '/test')
    cpath = os.getcwd()
    os.chdir(data + '/test')
    # os.chdir(cpath + '/tests/skysampler')
    yield data + '/test'
    # yield cpath + '/tests/skysampler'
    os.chdir(cpath)
...
def test_sample(tmpdir):
    ...
    assert np.isclose(np.average([illum, illumm, illum2, illumm2]), 3, atol=.1, rtol=.2)
